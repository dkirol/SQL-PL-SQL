...
-- FUNKCJA który resetuje haslo, po zatwierdzeniu i zmianie w tabeli z resetem hasla
...
--FUNKCJA wysylajaca email do resetu hasla
...
ELSE
    ...
    BEGIN
        ...
        INSERT INTO contacts (id_employee, city, street, num, phone, mail) VALUES (v_id_emplo_custo, city, street, num, phone, mail);
    END;
...
second_date DATE := '19/09/30';
count_free_day INTEGER := 0;
BEGIN
    WHILE first_date <= second_date + 1 LOOP
    
    DBMS_OUTPUT.PUT_LINE(TO_CHAR(first_date, 'DY'));

        IF TO_CHAR(first_date, 'DY') IN ('SO', 'N ')  THEN
            NULL;
        ELSE
            count_free_day := count_free_day + 1;
            
        END IF;
        first_date := first_date + 1;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE(count_free_day);
END;
